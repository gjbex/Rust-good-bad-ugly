...
::: notes
Run this after the byte-wise nucleotide example to contrast byte-oriented and
...
::: notes
Connect this to the earlier point that general text often needs line-based or
...
::: notes
This mirrors the earlier `Vec<T>` versus borrowed slice discussion.
:::
...
::: notes
Point out that timestamps contain colons, so parsing assumptions matter.
:::
...
::: notes
Keep this pragmatic: use a crate when the standard library intentionally does
not cover rich calendar/date-time parsing.
:::
...
::: notes
This prepares the transition to Module 8 without trying to teach all error
handling details here.
:::
...
::: notes
This is the structural matching connection requested by the module text.
:::
...
::: notes
Use this after the basic string parser. The data stays the same, so the
contrast is the parser shape.
:::
...
::: notes
Point out that patterns can validate and bind at the same time. Also point out
that this is only worth it when the pattern improves readability.
:::
...
- Lazy CSV scans
- Join metadata and measurements
- Filter missing temperatures
- Derive columns and compute grouped summaries
- Optional Parquet output

::: notes
Present this as optional extended material, not another numbered module. The
example mirrors a patient-data workflow from the Python data-science training.
Its use of `Result`, `?`, and separate library and binary targets makes it most
appropriate to revisit after Modules 8 and 9.
Build the release binary before the session because the initial Polars compile
is substantial.
:::

## Iterator Pipelines To Column Expressions
...
- Operations describe whole columns
- `collect()` executes the optimized plan
- Join and null rules are data-quality decisions

::: notes
Connect lazy DataFrame expressions to the iterator pipelines participants have
just learned. Stress that patient 4 has measurements but no metadata and is
therefore removed by the inner join. The query is easy to write; deciding and
reporting whether that exclusion is scientifically valid remains the analyst's
responsibility.
:::
...
9. Run `source-code/strings`
10. Change a timestamp or numeric field
11. Remove a field and inspect the parse error
12. Run `source-code/structural-matching`
13. Compare the two parser implementations
14. Optional after Modules 8 and 9: inspect the Polars query plan
