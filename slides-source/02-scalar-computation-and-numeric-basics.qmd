...
- Use `uom` for typed physical quantities at boundaries
...
- Plain `f64` values do not carry physical meaning
- `uom` gives quantities distinct types
- Keep units explicit at input and configuration boundaries

::: notes
Connect this to ODE and simulation setup: `t0`, `delta_t`, and `x0` are all
numbers, but they are not interchangeable quantities.
:::
...
- `clap` parses ordinary numeric values
- Conversion happens once near the boundary
- The solver can receive typed quantities

::: notes
Emphasize that the command-line interface stays simple: values are documented
as seconds and meters, then converted into typed quantities.
:::

## Terminal: Unit-Aware Parameters
...
- Inspect the printed `SimulationParameters`
- Locate `Length::new::<meter>`
- Locate `Time::new::<second>`

::: notes
Open `source-code/units/src/main.rs` after running the command. Point out that
the example converts back to canonical `f64` values before the simple loop.
:::
...
7. Run `source-code/units` and change `--delta-t`
...
- Where should unit conversions happen in a simulation?
