...
* compare equivalent Rust and C++ implementations of the same simulation;
...
1. `rust/src/main.rs`: main source file for the application.  It parses command-line
   arguments, runs the simulation, and optionally writes CSV files for the
   energy evolution and particle states.
1. `rust/src/system.rs`: definition and implementation of the `System` struct.  It
...
1. `cpp/src/main.cpp`: C++ application entry point.  It parses the same
   command-line arguments as the Rust version, runs the simulation, and
   optionally writes the same CSV outputs.
1. `cpp/src/system.h` and `cpp/src/system.cpp`: C++ declaration and
...
1. `rust/Cargo.toml`: configuration file for the Rust package manager.  It specifies
   the dependencies on `clap`, `csv`, `rand`, `rand_distr`, and `serde`.
1. `rust/Cargo.lock`: lock file for the Rust package manager, automatically
...
Compare the Rust and C++ implementations with release builds using `hyperfine`:
