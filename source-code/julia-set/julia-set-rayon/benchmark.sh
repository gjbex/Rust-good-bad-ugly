...
binary="$ROOT_DIR/target/release/$package_name"
if [[ ! -x "$binary" ]]; then
...
for threads in $THREAD_COUNTS; do
    if ! [[ "$threads" =~ ^[1-9][0-9]*$ ]]; then
        echo "error: THREAD_COUNTS must contain positive integers, got '$threads'" >&2
        exit 1
    fi

    printf -v command 'RAYON_NUM_THREADS=%q %q --width %q --height %q --max-iterations %q --c-real=%q --c-imag=%q > /dev/null' \
        "$threads" "$binary" "$WIDTH" "$HEIGHT" "$MAX_ITERATIONS" "$C_REAL" "$C_IMAG"
    commands+=("$command")
done

echo "Running Rayon scaling benchmarks"
echo "  warmup:         $WARMUP"
echo "  runs:           $RUNS"
echo "  width:          $WIDTH"
echo "  height:         $HEIGHT"
