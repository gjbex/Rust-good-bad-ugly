...
fn parse_record(record_str: &str) -> Result<Record, String> {
    ...
    for line in record_str.lines() {
        match line.split_once(':') {
            ...
            Some(("temperature", value)) => match value.trim().parse::<f64>() {
                Ok(temperature_val) => temperature = Some(temperature_val),
                Err(_) => return Err(format!("Failed to parse temperature: {}", value.trim())),
            },
            Some(("pressure", value)) => match value.trim().parse::<f64>() {
                Ok(pressure_val) => pressure = Some(pressure_val),
                Err(_) => return Err(format!("Failed to parse pressure: {}", value.trim())),
            },
            Some(_) | None => {}
        }
    }
