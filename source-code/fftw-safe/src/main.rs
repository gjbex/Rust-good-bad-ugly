...
#[derive(Debug, Parser)]
#[command(version, about = "Transform a test signal with the fftw crate")]
struct Args {
...
fn forward_and_inverse(signal: &[f64]) -> Result<(Vec<c64>, Vec<f64>), Box<dyn Error>> {
    let length = signal.len();
    let spectrum_length = length / 2 + 1;

    let mut input = AlignedVec::new(length);
    input.copy_from_slice(signal);
    let mut spectrum = AlignedVec::new(spectrum_length);
    let mut forward_plan = R2CPlan64::aligned(&[length], Flag::ESTIMATE)?;
    forward_plan.r2c(&mut input, &mut spectrum)?;

    let spectrum = spectrum.to_vec();
    let mut inverse_input = AlignedVec::new(spectrum_length);
    inverse_input.copy_from_slice(&spectrum);
    let mut inverse_output = AlignedVec::new(length);
    let mut inverse_plan = C2RPlan64::aligned(&[length], Flag::ESTIMATE)?;
    inverse_plan.c2r(&mut inverse_input, &mut inverse_output)?;

    let normalization = length as f64;
    let reconstructed = inverse_output
        .iter()
        .map(|value| value / normalization)
        .collect();

    Ok((spectrum, reconstructed))
}

fn one_sided_power_spectrum(spectrum: &[c64], signal_length: usize) -> Vec<f64> {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (spectrum, reconstructed) = forward_and_inverse(&signal)?;
    ...
    println!("Non-redundant spectrum length: {}", spectrum.len());
...
mod tests {
    ...
    fn detects_frequencies_and_reconstructs_the_signal() {
        let signal = create_signal(SIGNAL_LENGTH);
        let (spectrum, reconstructed) = forward_and_inverse(&signal).unwrap();
        let power_spectrum = one_sided_power_spectrum(&spectrum, signal.len());
